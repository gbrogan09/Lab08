...
int main()
{
    ...
    int score(0);
    int arrows(5);
    int framesToSpawnBalloon = 20;
    int currentFrame = 0;
    bool updatingPhysics = true;
    ...
    left.setCenter(Vector2f(-55, 300));
    left.setStatic(true);
    world.AddPhysicsBody(left);

    PhysicsRectangle right;
    right.setSize(Vector2f(10, 600));
    right.setCenter(Vector2f(855, 300));
    ...
    for (int i(0); i < 6; i++) {
        ...
        balloon.onCollision =
            [&drawingArrow, &world, &arrow, &right, &balloon, &balloons, &score, &popSound]
    ...
    while (true) {
        while ((arrows > 0) || drawingArrow) {
            ...
            if (deltaMS > 32) {
                ...
                world.UpdatePhysics(deltaMS / 2);
                MoveCrossbow(crossBow, deltaMS);
                if (Keyboard::isKeyPressed(Keyboard::Space) &&
                    !drawingArrow && arrows != 0) {
                    drawingArrow = true;
                    arrows = arrows - 1;
                    arrow.setCenter(crossBow.getCenter());
                    arrow.setVelocity(Vector2f(0, -2));
                    world.AddPhysicsBody(arrow);

                }
                currentFrame++;
                if (currentFrame >= framesToSpawnBalloon)
                {
                    currentFrame = 0;
                ...
                scoreText.setPosition(Vector2f(10, 580 - GetTextSize(scoreText).y));
